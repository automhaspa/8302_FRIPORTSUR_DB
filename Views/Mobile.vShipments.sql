...
CREATE VIEW [Mobile].[vShipments] AS
SELECT
...
L.label_aggiuntive.value('data(//Notes)[1]', 'VARCHAR(MAX)') AS Notes -- Informazioni da definire NVCHAR(100) in versione originale
FROM dbo.Liste AS L
WHERE l.Id_Tipo_Lista = 'TDO' AND L.Id_Tipo_Stato_Lista<>7 -- lista non chiusa
GO
