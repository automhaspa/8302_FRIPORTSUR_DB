...
CREATE VIEW [dbo].[vUdc_Baia_Outbound]
AS
WITH Udc_Interessati 
AS 
(
...
SELECT      UC.Id_Partizione AS [Id Partizione], 
			UC.Id_Missione AS [Id Missione], 
			UC.Id_Udc AS [Id Udc], 
            UT.Codice_Udc AS Codice,
			UT.Larghezza AS Larghezza, 
			UT.Profondita AS Profondità, 
            UT.Altezza AS Altezza,
			UT.Peso AS Peso,  
			PD.DESCRIZIONE as [Posizione di Origine],
			PC.DESCRIZIONE AS [Posizione Attuale], 
			M.Id_Partizione_Destinazione AS [Id Destinazione],
			PS.DESCRIZIONE as [Partizione di Destinazione], 			
		    M.Id_Tipo_Missione as [Tipo Missione],
			M.Id_Stato_Missione AS [Stato Missione]
