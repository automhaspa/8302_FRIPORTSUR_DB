...
CREATE VIEW [AwmConfig].[vPassiInCorso]
AS
WITH Error_Steps AS
(
...
SELECT	P.Id_Missione,
		...
		TSP.Descrizione						Stato,
		EP.Errore							Errore,
		CASE
			WHEN P.Id_Tipo_Stato_Percorso IN (2,9) THEN DATEDIFF(SECOND,P.Ultima_Esecuzione,GETDATE())
			ELSE NULL
		END									Secondi_Esecuz
...
WHERE	P.Id_Tipo_Stato_Percorso IN (2,9)
GO
