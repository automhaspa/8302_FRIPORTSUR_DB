...
(
    ...
    FROM	dbo.Esito_Passi_Mappatura
    GROUP 
	BY		Id_Piano_Mappatura
),
InfoNumericheSulleMissioni AS
(
    SELECT  pm.Id_Piano_Mappatura, 			 
				( MIN(pm.A_Colonna)    +1 - MIN(pm.Da_Colonna)    )
			   *( MIN(pm.A_Piano)      +1 - MIN(pm.Da_Piano)      )
			   *( MIN(pm.A_Profondita) +1 - MIN(pm.Da_Profondita) )
			 
												AS NumeroTotaleMissioni,
			MIN(nmppm.EsitiMissioniInErrore)	AS EsitiMissioniInErrore,
			MIN(nmppm.MissioniSvolte)			AS MissioniSvolte		
    FROM	dbo.Piani_Mappature pm
    LEFT 
	JOIN	NumeroMissioniPerPianoMappatura nmppm 
    ON		pm.Id_Piano_Mappatura = nmppm.Id_Piano_Mappatura
    GROUP 
	BY		pm.Id_Piano_Mappatura
)
SELECT	pim.Id_Piano_Mappatura,
